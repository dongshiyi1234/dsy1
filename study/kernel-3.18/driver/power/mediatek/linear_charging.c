...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	if (g_temp_status == TEMP_ABOVE_POS_60) {
		...
		// modified by elink_phil start <<<
			cv_voltage = CONSTANT_VOLTAGE * 1000;
		else
			cv_voltage = CONSTANT_VOLTAGE * 1000;
		// modified end >>>
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			if (batt_cust_data.high_battery_voltage_support)
			// modified by elink_phil start <<<
				g_jeita_recharging_voltage = RECHARGING_VOLTAGE;
			else
...
static void set_jeita_charging_current(void)
{
	...
	if (g_temp_status == TEMP_BELOW_NEG_10 || g_temp_status == TEMP_NEG_10_TO_POS_0 || BMT_status.temperature == 0) {
		g_temp_CC_value = 0;
	...
	{
		qmax = Q_MAX_POS_0 * 100 + (Q_MAX_POS_25 - Q_MAX_POS_0) * BMT_status.temperature * 100 / 25;	// *100 means to match array value in charger_hw*.c, if 1A, value is 100000
		qmax = qmax / 5; // max is 0.2c

		if (g_temp_CC_value > qmax)
		{
			g_temp_CC_value = qmax;
			g_temp_CC_value = qmax;
			
			battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d, battery temp is %d, 0~10 degree, so max charging current is 0.2 Qmax\r\n",
			    g_temp_CC_value, BMT_status.temperature);
		}
	}
	else
	{
		if (BMT_status.temperature <= 25)
			qmax = Q_MAX_POS_0 * 100 + (Q_MAX_POS_25 - Q_MAX_POS_0) * BMT_status.temperature * 100 / 25;
		else
			qmax = Q_MAX_POS_25 * 100 + (Q_MAX_POS_50 - Q_MAX_POS_25) * BMT_status.temperature * 100 / 25;
		
		qmax = qmax / 2; // max is 0.5c
		...
		if (g_temp_CC_value > qmax)
		{
			...
			battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d, battery temp is %d, up 10 degree, so max charging current is 0.5 Qmax\r\n",
			    g_temp_CC_value, BMT_status.temperature);
		}
	}
}
...
// modified by elink_phil start <<<
extern void elink_set_recharge_info(void);
// modified end >>>
