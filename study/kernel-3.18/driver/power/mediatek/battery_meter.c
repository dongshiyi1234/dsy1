...
// added by elink_phil for elink battery start <<<
#include <elink_battery/elink_battery.h>
#include <elink_battery/elink_battery_list.h>
...
int __batt_meter_init_cust_data_from_cust_header(void)
{
	...
	// modified by elink_phil for elink battery start <<<
	/* Qmax for battery  defined in file elink_battery.h*/
...
#ifndef EXTERNAL_SWCHR_SUPPORT
#ifdef CONFIG_ELINK_CUST_R_SENSE
	batt_meter_cust_data.cust_r_sense = CONFIG_ELINK_CUST_R_SENSE;
	printk("%s: get rsense from kernel config is %d\n", __func__, batt_meter_cust_data.cust_r_sense);
#endif
#endif
#endif
...
#define PASSWD "phil"
static char opcode[16] = {'\0'}; // password:phil
static int temp_table[MAX_TEMP_NUM] = {-10, 0, 25, 50};
static int real_qmax[MAX_TEMP_NUM];

#ifdef CONFIG_BATTERY_NTC_SUPPORT
static BATTERY_PROFILE_STRUCT *bat_table[MAX_TEMP_NUM];
static R_PROFILE_STRUCT *r_table[MAX_TEMP_NUM];
#else
static BATTERY_PROFILE_STRUCT *bat_table[MAX_TEMP_NUM];
static R_PROFILE_STRUCT *r_table[MAX_TEMP_NUM];
#endif

static int battery_debug(char *buf, unsigned long index)
{
	int i = 0;
	int size = 0;
	int total = 0;
	int temp = temp_table[index];
	
#ifdef CONFIG_BATTERY_NTC_SUPPORT
	real_qmax[0] = batt_meter_cust_data.q_max_neg_10;
	real_qmax[1] = batt_meter_cust_data.q_max_pos_0;
	real_qmax[2] = batt_meter_cust_data.q_max_pos_25;
	real_qmax[3] = batt_meter_cust_data.q_max_pos_50;
	
	bat_table[0] = battery_profile_t0;
	bat_table[1] = battery_profile_t1;
	bat_table[2] = battery_profile_t2;
	bat_table[3] = battery_profile_t3;
	
	r_table[0] = r_profile_t0;
	r_table[1] = r_profile_t1;
	r_table[2] = r_profile_t2;
	r_table[3] = r_profile_t3;
#else
	for (i = 0; i < MAX_TEMP_NUM; i++)
	{
		real_qmax[i] = batt_meter_cust_data.q_max_pos_25;
		bat_table[i] = battery_profile_t2;
		r_table[i] = r_profile_t2;
	}
#endif

#ifndef CONFIG_BATTERY_NTC_SUPPORT
	index = 0;
	temp = 25;
#endif

	size = sprintf(buf, "qmax = %d\n", real_qmax[index]);
	buf += size;
	total += size;
	
	size = sprintf(buf, "<DOD,Voltage>\n");
	buf += size;
	total += size;

	for (i = 0; i < PROFILE_TABLE_SIZE; i++)
	{
		size = sprintf(buf, "%dC = <%d,%d>\n", temp, (bat_table[index] + i)->percentage, (bat_table[index] + i)->voltage);
		buf += size;
		total += size;
	}
	
	size = sprintf(buf, "<Rbat,VBAT>\n");
	buf += size;
	total += size;

	for (i = 0; i < PROFILE_TABLE_SIZE; i++)
	{
		size = sprintf(buf, "%dC = <%d,%d>\n", temp, (r_table[index]  + i)->resistance, (r_table[index] + i)->voltage);
		buf += size;
		total += size;
	}
...
static ssize_t show_battery_info(struct device *dev,
						    struct device_attribute *attr, char *buf)
{
	int total = 0;
	...
	if (strncmp(opcode, PASSWD, strlen(PASSWD)) == 0)
	{
		if (*(opcode + strlen(PASSWD)) != '\0')
		{
			temp = simple_strtoul((opcode + strlen(PASSWD)), &after, 10);
			printk("%s: temp = %ld\n", __func__, temp);
		}
		
		if (temp >= 0 && temp < MAX_TEMP_NUM)
			total = battery_debug(buf, temp);
...
static ssize_t store_battery_info(struct device *dev,
						     struct device_attribute *attr, const char *buf,
						     size_t size)
{
	if (size > 16)
		size = 16;
	memcpy(opcode, buf, size);
	
	return size;
}

static DEVICE_ATTR(battery_info, 0664, show_battery_info, store_battery_info);
#define CHR_CUR_COUNT 6
#define CHR_REG_COUNT 3
typedef struct {
	int avg;
	int real;
}CURRENT_INFO;
...
typedef struct {
	...
	int jeita;
	int ov_occur;
	CURRENT_INFO chr_cur[CHR_CUR_COUNT];
	int rsense;
	int full;
	RECHARGE_INFO rechr;
	REGISTER_INFO reg[CHR_REG_COUNT];
}BATTERY_FLAG;
BATTERY_FLAG g_bt_flag = {
...
#endif
	.ov_occur = 0,
	.chr_cur = {{.avg = CONFIG_ELINK_CHARGING_CURRENT, .real = CONFIG_ELINK_CHARGING_CURRENT}},
	.rsense = CONFIG_ELINK_CUST_R_SENSE,
	.rechr = {.vol = RECHARGING_VOLTAGE, .soc = 0},
	.reg = {
		{.reg = CHR_CON3, .val = 0},
		{.reg = CHR_CON4, .val = 0},
		{.reg = CHR_CON6, .val = 0},
...
void elink_set_ov_flag(void)
{
	g_bt_flag.ov_occur = 1;
}
EXPORT_SYMBOL(elink_set_ov_flag);
void elink_set_chr_current(int avg, int real)
{
	static int index = 0;
	if (real != 0)
	{
		printk("%s: index = %d, real cur = %d\n", __func__, index, real);
		g_bt_flag.chr_cur[index].avg = avg;
		g_bt_flag.chr_cur[index++].real = real;
		if (index == CHR_CUR_COUNT)
		{
			index = 0;
		}
	}
}
EXPORT_SYMBOL(elink_set_chr_current);
void elink_set_recharge_info(void)
{
	if (g_bt_flag.rechr.soc == 0)
		g_bt_flag.rechr.soc = BMT_status.SOC;
	else
	{
		if (100 - g_bt_flag.rechr.soc <= 10)
			g_bt_flag.rechr.soc = BMT_status.SOC;
	}
}
EXPORT_SYMBOL(elink_set_recharge_info);
#define elink_append_sysfs(arg1, arg2, arg3, fmt...) do { \
	arg3 = sprintf(buf, fmt); \
	arg1 += arg3; \
	arg2 += arg3; \
}while(0)
static ssize_t show_battery_flag(struct device *dev,
						    struct device_attribute *attr, char *buf)
{
	int total = 0;
	int size = 0;
	int i;
	elink_append_sysfs(buf, total, size, "high_vol = %d\n", g_bt_flag.high_vol);
	elink_append_sysfs(buf, total, size, "ntc_exists = %d\n", g_bt_flag.ntc_exists);
	elink_append_sysfs(buf, total, size, "jeita = %d\n", g_bt_flag.jeita);
	elink_append_sysfs(buf, total, size, "ov_occur = %d\n", g_bt_flag.ov_occur);
	elink_append_sysfs(buf, total, size, "chr_cur =");
	for (i = 0; i < CHR_CUR_COUNT; i++)
	{
		elink_append_sysfs(buf, total, size, " (%d, %d)", g_bt_flag.chr_cur[i].avg, g_bt_flag.chr_cur[i].real);
	}
	elink_append_sysfs(buf, total, size, "\n");
	elink_append_sysfs(buf, total, size, "rsense = %d\n", g_bt_flag.rsense);
	elink_append_sysfs(buf, total, size, "bat_full = %d\n", BMT_status.bat_full);
	elink_append_sysfs(buf, total, size, "rechr = %d, soc = %d\n", g_bt_flag.rechr.vol, g_bt_flag.rechr.soc);
	elink_append_sysfs(buf, total, size, "chr reg =");
	for (i = 0; i < CHR_REG_COUNT; i++)
	{
		elink_append_sysfs(buf, total, size, " [0x%x, 0x%x]", g_bt_flag.reg[i].reg, upmu_get_reg_value(g_bt_flag.reg[i].reg));
	}
	return total;
}

static DEVICE_ATTR(battery_flag, 0664, show_battery_flag, NULL);
// added end >>>
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_battery_info);
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_battery_flag);
...
static int __init battery_meter_init(void)
{
	int ret;
// added by elink_phil for elink battery start <<<
	ELINK_BATTERY_FLAG flag;
#ifdef CONFIG_BATTERY_NTC_SUPPORT
	flag.ntc_exists = 1;
#ifdef BATTERY_NTC_RESIST
	flag.ntc_resist = BATTERY_NTC_RESIST;
#else
	flag.ntc_resist = 10;
#endif
	...
	elink_get_battery_info(battery_list, sizeof(battery_list) / sizeof(ELINK_BATTERY_INFO), flag);
// added end >>>
#ifdef CONFIG_OF
	/*  */
#else
