...
// added by elink_phil@20171016 for dc charge start <<<
#ifdef CONFIG_ELINK_DC_CHARGE_SUPPORT
#include <elink_dc_charge.h>
#endif
// added end >>>
...
// added by elink_wenxt@20180807 start >>>>
#ifdef CONFIG_ELINK_DC_USB_WITHOUT_IRQ
extern int get_otg_state(void);
#endif
// added end
...
// added by elink_phil@20171016 for dc charge start <<<
#ifdef CONFIG_ELINK_DC_CHARGE_SUPPORT
	if (dc_charge_info.state & (DC_ONLINE | USB_ONLINE))
		return KAL_TRUE;
#endif
// added end >>>
	...
	if (tmp32 == 0)
		return KAL_FALSE;
// added by elink_wenxt start >>>>
#if defined(CONFIG_ELINK_PCBA_H115)
		return KAL_TRUE;
#endif
// added end <<<<
	/*else {*/
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	#ifdef SOC_BY_HW_FG
		if (BMT_status.ZCV > SYSTEM_OFF_VOLTAGE && BMT_status.UI_SOC > 1)
			BMT_status.UI_SOC--;
		else if (BMT_status.ZCV <= SYSTEM_OFF_VOLTAGE)
			BMT_status.UI_SOC--;
	#else
...
// modified by elink_phil start <<<
extern void elink_set_chr_current(int avg, int real);
// modified end >>>
void mt_battery_GetBatteryData(void)
{
	...
	// modified by elink_phil start <<<	
	elink_set_chr_current(BMT_status.ICharging, ICharging);
	// modified end >>>
...
#else
// modified by elink_phil@20171016 for dc charge start <<<
#if defined(CONFIG_ELINK_DC_CHARGE_SUPPORT)
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
		    (dc_charge_info.state & USB_ONLINE)) {
#elif defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
...
*/
...
#ifdef CONFIG_ELINK_DC_USB_WITHOUT_IRQ
		otg_state = get_otg_state();
		if((g_platform_boot_mode != META_BOOT)&&(!otg_state)) {
			BMT_status.charger_type = NONSTANDARD_CHARGER;
		}
#endif
		// added end <<<<
	...
	// added by elink_phil@20171016 for dc charge start <<<
	#ifdef CONFIG_ELINK_DC_CHARGE_SUPPORT
		dc_charge_info.charger_type = CHARGER_UNKNOWN;
	#endif
	// added end >>>
		BMT_status.bat_full = KAL_FALSE;
	...
	// added by elink_phil@20171016 for dc charge start <<<
	#ifdef CONFIG_ELINK_DC_CHARGE_SUPPORT
		if (!(dc_charge_info.state & OTG_ONLINE))
	#endif
	// added end >>>
	// added by elink_wenxt start >>>>
#ifdef CONFIG_ELINK_DC_USB_WITHOUT_IRQ
...
// added by elink_phil@20171016 for dc charge start <<<
#ifdef CONFIG_ELINK_DC_CHARGE_SUPPORT
CHARGER_TYPE elink_charger_type_detection(void)
{
	CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;
...
void vin_detect_callback(void *para)
{
	if (dc_charge_info.state & DC_ONLINE)
	{
		dc_charge_info.charger_type = BMT_status.charger_type;
		BMT_status.charger_type = NONSTANDARD_CHARGER;
	}
	else
	{
		if (dc_charge_info.state & USB_ONLINE)
		{
			BMT_status.charger_type = dc_charge_info.charger_type;
		}
	}

	do_chrdet_int_task();

	pr_debug("%s : dc_charge_info charger_type = %d\n", __func__, dc_charge_info.charger_type);
}

void otg_detect_callback(void *para)
{
	if (dc_charge_info.state & USB_ONLINE)
	{
		mutex_lock(&charger_type_mutex);
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &dc_charge_info.charger_type);
		mutex_unlock(&charger_type_mutex);
		printk("%s : dc_charge_info charger_type = %d\n", __func__, dc_charge_info.charger_type);
		...
		if ((dc_charge_info.charger_type == STANDARD_HOST)
			|| (dc_charge_info.charger_type == CHARGING_HOST))
		{
			printk("%s : call mt_usb_connect\n", __func__);
			mt_usb_connect();
		}
	}
	else
	{
		if (dc_charge_info.state & DC_ONLINE)
		{
			...
			|| (dc_charge_info.charger_type == CHARGING_HOST))
			{
				printk("%s : call mt_usb_disconnect\n", __func__);
				mt_usb_disconnect();
			}
		}
	}
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			BMT_status.charger_exist = KAL_FALSE;
// modified by elink_phil@20171016 for dc charge start <<<
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT) || defined(CONFIG_ELINK_DC_CHARGE_SUPPORT)
			battery_log(BAT_LOG_CRTI,
...
#endif
// modified by elink_phil start <<<
void elink_init_customer_setting(void)
{
#ifndef EXTERNAL_SWCHR_SUPPORT
	int rsense;
#endif

#ifdef CONFIG_ELINK_CHARGING_CURRENT
	batt_cust_data.ac_charger_current = CONFIG_ELINK_CHARGING_CURRENT * 100;
	batt_cust_data.non_std_ac_charger_current = CONFIG_ELINK_CHARGING_CURRENT * 100;

	printk("%s: charging current from kernel config is %d, %d\n", __func__, batt_cust_data.ac_charger_current, batt_cust_data.non_std_ac_charger_current);
#endif
	...
	if (rsense == 100)
	{
		batt_cust_data.ac_charger_current /= 2;
		batt_cust_data.non_std_ac_charger_current /= 2;
		batt_cust_data.usb_charger_current /= 2;
	}

	printk("%s: charging current is %d, %d\n", __func__, batt_cust_data.ac_charger_current, batt_cust_data.non_std_ac_charger_current);
#endif
}
// modified end >>>
...
// added by elink_phil@20171016 for dc charge start <<<
#ifdef CONFIG_ELINK_DC_CHARGE_SUPPORT
	dc_charge_info.vin_sel.callback = vin_detect_callback;
	dc_charge_info.otg_det.callback = otg_detect_callback;
	
	complete_all(&dc_charge_info.completion);
#endif
// added end >>>
