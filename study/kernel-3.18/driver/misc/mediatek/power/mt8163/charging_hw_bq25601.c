...
static unsigned int charging_hw_init(void *data)
{
    ...
    bq25601_set_vindpm(0x5); //VIN DPM check 4.4V
    ...
    bq25601_set_iterm(0x4);    //Termination current 240mA
	if (batt_cust_data.high_battery_voltage_support)
    	bq25601_set_vreg(0xF);  /* VREG 4.352V(4336) */
	else
    ...
    bq25601_set_ovp(0x0); //0x01//Battery overvoltage Protection 6.5v
    //bq25601_set_boostv(0x2); //Boost Regulation Voltage 5.15v
    bq25601_set_vindpm_int_mask(0x0); //Mask VINDPM INT pulse
    bq25601_set_iindpm_int_mask(0x0); //Mask IINDPM INT pulse

    bq25601_set_batfet_rst_en(0x0);
	...
	if (KAL_TRUE == enable) {
		...
		if (mt_usb_is_device()) {
			...
			if (charging_get_error_state()) {
				...
				bq25601_set_en_hiz(0x1);	/* disable power path */
			}
#if defined(CONFIG_USB_MTK_HDRC_HCD)
		}
#endif
...
static unsigned int charging_set_ta_current_pattern(void *data)
{
	...
	if (KAL_FALSE == charging_status) {
		...
		bq25601_set_ichg(0x9);	/* Set charging current 500ma */
